...
class NewYearReplacementTable extends PowerGridComponent
{
    ...
    {
        $query =  HolidayReplacement::query()
            ->leftJoin('alchemy_replacement_tutor', function ($replacement_tutor) {
                $replacement_tutor->on('alchemy_holiday_replacement.replacement_id', '=', 'alchemy_replacement_tutor.id');
            })
            ->leftJoin('tutors', function ($tutor) {
                $tutor->on('tutors.id', '=', 'alchemy_replacement_tutor.tutor_id');
            })
            ->leftJoin('alchemy_children', function($child) {
                $child->on('alchemy_children.id', '=', 'alchemy_replacement_tutor.child_id');
            })
        ...
        return $query->select('alchemy_holiday_replacement.*');
    }
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('child_name', fn ($item) =>  $item->replacement_tutor->child->child_name ?? '-')
            ->add('parent_first_name', fn ($item) =>  $item->replacement_tutor->parent ? $item->replacement_tutor->parent->parent_first_name . ' ' . $item->child->parent->parent_last_name : '-')
            ->add('parent_email', fn ($item) =>  $item->replacement_tutor->parent->parent_email ?? '-')
            ->add('tutor_name', fn ($item) =>  $item->replacement_tutor->tutor->tutor_name ?? '-')
